...
- Campaign is the default mode and follows a sixteen-level story map. It starts with one system, introduces the others gradually, then unlocks additional procedures, palettes, reading directions, and stronger pressure. Campaign progress, viewed intermissions, recovered archive fragments, and per-level best scores are saved automatically in the host browser. The campaign lobby can copy a checksummed `CHD2` recovery code and restore the complete story state in another browser; older `CHD1` codes remain compatible, and an interrupted level restarts from its beginning.
