...
function App() {
  ...
  if (screen === 'intermission') return <NightShiftDesktop view={view} campaignStory={campaignStory} isHost={isHost} onDialogueChoice={chooseDialogue} onContinue={startNextCampaignLevel} onLeave={leaveSession} />
...
function OperatorConsole({ view: fullView, onAction }: { view: GameView; onAction: (action: GameActionInput) => void }) {
  ...
  return <div className="game-content"><div className="role-banner"><div><p className="kicker">{t.assignment}</p><h1>{t.operatorConsole}</h1><small className="phase-indicator">{t.missionPhase} {fullView.phaseIndex + 1}/{fullView.phaseCount}</small></div><p>{t.operatorSubtitle}<br /><b>{t.describe}</b></p></div><CampaignStory view={fullView} /><section className={`module-grid modules-${Math.max(1, view.activeModules.length)}`}>
    ...
    {view.activeModules.includes('triage') && <article data-resolved={t.resolved} className={`module-card triage-card ${data.triage.resolved ? 'resolved' : ''}`}><ModuleHeader number="01" title={t.powerTriage} resolved={data.triage.resolved} tone="orange" language={view.language} /><div className="power-budget"><span>{t.powerBudget}</span><strong>{powerTotal} / {data.triage.budget}</strong></div><p className="instruction">{t.powerInstruction}</p><div className="power-habitats">{data.triage.habitats.map(habitat => <div key={habitat.id}><span>{habitat.label}</span><section><button disabled={data.triage.resolved || (powerAllocations[habitat.id] || 0) === 0} onClick={() => adjustPower(habitat.id, -1)}>−</button><strong>{powerAllocations[habitat.id] || 0}</strong><button disabled={data.triage.resolved || powerTotal >= data.triage.budget} onClick={() => adjustPower(habitat.id, 1)}>+</button></section></div>)}</div><button className="module-submit orange" disabled={data.triage.resolved || powerTotal !== data.triage.budget} onClick={() => { onAction({ type: 'triage-submit', allocations: data.triage.habitats.map(habitat => ({ habitatId: habitat.id, units: powerAllocations[habitat.id] || 0 })) }); setPowerAllocations({}) }}>{t.commitPower}</button></article>}
...
function desktopMessages(levelId: number, language: Locale, campaignStory: CampaignStoryProgress): DesktopMessage[] {
  ...
  const scripted: Record<number, DesktopMessage[]> = {
    ...
    9: [
      { sender: 'MARA VALE // DEAD LETTER', body: de ? 'Das Netz kopiert bewohnte Welten. Die Koordinaten führen zum Relaisfriedhof. Wenn ihr das lest, bin ich noch nicht zu Hause.' : 'The network is copying inhabited worlds. The coordinates lead to the relay graveyard. If you read this, I am not home yet.', tone: 'mara' },
    ...
    16: [
      ...
      { sender: 'ARCHIVE 404', body: de ? 'SCHUTZ KOPIERTER ZIVILISATIONEN: AKTIV // AKTE DER STILLEN VERSAMMLUNG: ÖFFENTLICH' : 'COPIED-CIVILIZATION PROTECTIONS: ACTIVE // QUIET ASSEMBLY RECORD: PUBLIC', tone: 'system' },
